...
from .util import BossError, locked, now, read_json, write_json
from . import ids
...
def cas_update(work_id: str, mutate: Callable[[dict], None], expected_revision: int | None = None) -> dict:
    ...
    with locked(item_lock(work_id)):
        ...
        if expected_revision is not None and revision != expected_revision:
            raise BossError(f"stale work item revision {expected_revision}; current revision is {revision}")
        mutate(item)
...
def redact(value):
    """Recursively redact every boss-visible field, including dict keys and history."""
...
def request(work_id: str, action: str, value=None, request_id: str | None = None) -> dict:
    ...
    if action not in allowed:
        raise BossError(f"unknown control '{action}'")
    if request_id is not None and not re.fullmatch(r"[A-Za-z0-9._:-]{1,128}", request_id):
        raise BossError("request id must be 1-128 safe identifier characters")
    ...
    def mutate(item):
        ...
        if promotion.get("state") in {
            ...
            raise BossError(f"{action} refused while an exact-SHA promotion is armed; reconcile promotion first")
        pr_delivery = item.get("pr_delivery") or {}
        if pr_delivery.get("state") in {
            "armed", "push-requested", "push-confirmed", "pr-create-requested"
        }:
            raise BossError(f"{action} refused while exact-SHA PR delivery is armed; reconcile delivery first")
        ...
        if external_gate.get("state") in {
            ...
            raise BossError(f"{action} refused while a no-mistakes transaction requires reconciliation")
        ...
        if existing:
            ...
            if existing.get("action") != action or existing.get("value") != expected_value:
                raise BossError(f"control request id '{event_id}' was already used for different content")
            return
        ...
        if action == "pause":
            ...
            if not str(value or "").strip():
                raise BossError("steer requires guidance")
...
def begin_delivery(work_id: str, event_id: str, owner: str, baseline_sequence: int,
    ...
    def mutate(item):
        ...
        if not event or event.get("action") != "steer":
            raise BossError(f"unknown steering event '{event_id}'")
...
def complete_delivery(work_id: str, event_id: str, owner: str, input_sequence: int) -> dict:
    def mutate(item):
        ...
        if not event or event.get("state") != "delivering" or event.get("delivery_owner") != owner:
            raise BossError("steering delivery ownership changed before acknowledgement")
