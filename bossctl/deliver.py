"""Delivery and boss-authorized, exact-SHA promotion transactions."""
...
from .util import BossError, git, git_config, locked, log, now, sh
...
def _verification(item: dict) -> dict:
    ...
    if not (_full_sha(head) and record.get("ok") is True and record.get("complete") is True):
        raise BossError("promotion refused: complete successful final verification is missing")
    if record.get("head_sha") != head or not _full_sha(record.get("base_sha")):
        raise BossError("promotion refused: verification is not bound to full exact head/base SHAs")
    fingerprint = record.get("fingerprint")
    if not isinstance(fingerprint, dict) or fingerprint.get("sha") != head or fingerprint.get("dirty") != []:
        raise BossError("promotion refused: clean exact-SHA worktree fingerprint is missing")
    return record
...
def _reviews(item: dict, verification: dict) -> None:
    ...
    if len(reviews) != len(required):
        raise BossError(f"promotion refused: expected {len(required)} exact-SHA review(s), found {len(reviews)}")
    by_role = {review.get("role"): review for review in reviews}
    if set(by_role) != set(required):
        raise BossError("promotion refused: required independent reviewer roles are incomplete or duplicated")
    for role in required:
        ...
        if (review.get("verdict") != "accept" or review.get("sha") != item.get("head_sha")
            ...
            raise BossError(f"promotion refused: {role} approval is not fresh and exact-SHA-bound")
        review_base = review.get("base_sha")
        if review_base != verification.get("base_sha"):
            raise BossError(f"promotion refused: {role} approval used a different base SHA")
...
def _worktree_path(item: dict, project: dict) -> Path:
    ...
    if recorded != expected:
        raise BossError("promotion refused: recorded worktree escapes the item's owned path")
    return expected
...
def _validate(item: dict, project: dict, *, require_worktree: bool = True) -> tuple[dict, Path]:
    ...
    if item.get("project") != project.get("id"):
        raise BossError("promotion refused: item/project binding changed")
    if item.get("status") not in ("ready", "pr-open"):
        raise BossError(f"{item['id']} is {item.get('status')}; only ready/pr-open items promote")
    ...
    if unresolved or controls.get("paused") or controls.get("pause_requested") or controls.get("interrupt_requested"):
        raise BossError("promotion refused: unresolved boss control must be reconciled first")

    wt = _worktree_path(item, project)
    if require_worktree and not wt.is_dir():
        raise BossError("promotion refused: verified worktree is missing; only an armed transaction may reconcile it")
    if wt.is_dir():
        if not worktree.is_clean(wt):
            raise BossError("worktree mutated after verification; approval is invalid")
        current = git(wt, "rev-parse", "HEAD", check=False)
        if current != item["head_sha"]:
            raise BossError("branch mutated after verification; approval is invalid")
        if worktree.signature(wt) != verification["fingerprint"]:
            raise BossError("worktree fingerprint changed after verification; approval is invalid")
        branch_sha = git(project["path"], "rev-parse", "--verify", item["branch"], check=False)
        if branch_sha != item["head_sha"]:
            raise BossError("promotion refused: item branch no longer names the reviewed exact SHA")
        ancestry = sh(["git", "-C", str(wt), "merge-base", "--is-ancestor",
                       verification["base_sha"], item["head_sha"]], check=False)
        if ancestry.returncode:
            raise BossError("promotion refused: reviewed base is not an ancestor of the exact head SHA")
    base_now = git(project["path"], "rev-parse", project["base"], check=False)
    if base_now != verification["base_sha"]:
        raise BossError("configured base moved after verification; recover the item to integrate and review again")
...
def after_success(it: dict, project: dict, wt: Path) -> None:
    ...
    it["head_sha"] = head
    if not worktree.is_clean(wt):
        raise BossError("delivery refused: worktree has uncommitted or untracked mutations")
    ...
    if len(reviews) != len(required) or any(
        ...
        raise BossError("independent approval is missing or not bound to the exact delivery SHA")
...
def _remote_head(project: dict, branch: str, origin: str | None = None) -> str | None:
    ...
    if not explicit_origin:
        raise BossError("remote branch observation has no configured origin")
    result = sh(["git", "-C", str(project["path"]), "ls-remote", "--heads",
                 explicit_origin, ref], check=False, network=True)
    if result.returncode:
        raise BossError("remote branch observation unavailable; PR delivery remains journaled")
    lines = [line.split() for line in result.stdout.splitlines() if line.strip()]
    if not lines:
        return None
    if (len(lines) != 1 or len(lines[0]) != 2 or lines[0][1] != ref
            or not _full_sha(lines[0][0])):
        raise BossError("remote branch observation was ambiguous or malformed")
    return lines[0][0]
...
def _query_pr(project: dict, delivery: dict) -> dict | None:
    ...
    if result.returncode:
        raise BossError("GitHub PR observation unavailable; delivery remains unresolved")
    try:
        rows = json.loads(result.stdout)
    except json.JSONDecodeError as exc:
        raise BossError("GitHub PR observation returned invalid JSON") from exc
    if not isinstance(rows, list) or any(not isinstance(row, dict) for row in rows):
        raise BossError("GitHub PR observation returned an invalid shape")
    ...
    if any(row not in exact for row in related):
        raise BossError("a GitHub PR for the item branch has a changed head or base; refusing to infer identity")
    if len(exact) > 1:
        raise BossError("multiple exact GitHub PRs matched one delivery journal")
    if not exact:
        return None
    row = exact[0]
    ...
    if not parsed or parsed[0].lower() != str(delivery["repository"]).lower():
        raise BossError("GitHub returned a non-canonical or foreign PR URL")
    if state != "OPEN":
        raise BossError(f"the exact delivery PR is {state or 'UNKNOWN'}; boss reconciliation is required")
...
def _validate_pr_side_effect(item: dict, project: dict, delivery: dict, wt: Path) -> None:
    ...
    if (active.get("id") != delivery.get("id") or active.get("state") not in _OPEN_PR_DELIVERY
        ...
        raise BossError("item or PR delivery identity changed before the external side effect")
    verification = _verification(item)
    _reviews(item, verification)
    if (verification.get("base_sha") != delivery.get("base_sha")
            or verification.get("fingerprint") != delivery.get("worktree_fingerprint")):
        raise BossError("verification evidence changed after PR delivery armed")
    controls = item.get("controls") or {}
    unresolved = [event for event in controls.get("events", [])
                  if event.get("state") in ("pending", "delivering")]
    if (unresolved or controls.get("paused") or controls.get("pause_requested")
            or controls.get("interrupt_requested")):
        raise BossError("PR delivery refused: unresolved boss control must be reconciled first")
    if (_pr_policy(project) != delivery.get("project_policy")
        ...
        raise BossError("registered project policy changed after PR delivery armed")
    gates.require_execution(project.get("gate", "native"), project["path"])
    if git_config(project["path"], "remote.origin.url") != delivery.get("origin"):
        raise BossError("origin changed after PR delivery armed")
    if git(project["path"], "rev-parse", project["base"], check=False) != delivery.get("base_sha"):
        raise BossError("base moved after review; PR delivery cannot create a new side effect")
    if not wt.is_dir() or worktree.signature(wt) != delivery.get("worktree_fingerprint"):
        raise BossError("worktree changed after review; PR delivery remains preserved")
    if (git(wt, "rev-parse", "HEAD", check=False) != delivery.get("head_sha")
        ...
        raise BossError("item branch changed after PR delivery armed")
...
def _pr_update(work_id: str, intent_id: str, expected_states: set[str], **changes) -> dict:
    ...
    def mutate(current):
        ...
        if active.get("id") != intent_id or active.get("state") not in expected_states:
            raise BossError("PR delivery journal changed during reconciliation")
...
def _arm_pr_delivery(item: dict, project: dict, wt: Path) -> dict:
    ...
    if not repository:
        raise BossError("origin is not a recognized GitHub repository; PR delivery refused")
    if worktree.signature(wt) != verification["fingerprint"]:
        raise BossError("worktree changed before PR delivery could arm")
    intent_id = secrets.token_hex(16)
    safe_text = str(control.redact(item["text"]))
    title = safe_text.strip().splitlines()[0][:70]
    body = (f"bossctl work item `{item['id']}`\n\n{safe_text}\n\n"
    ...
    def arm(current):
        if current.get("status") not in {"running", "failed", "queued"} or current.get("head_sha") != item.get("head_sha"):
            raise BossError("item changed before PR delivery could arm")
        prior = current.get("pr_delivery") or {}
        if prior.get("state") in _OPEN_PR_DELIVERY:
            raise BossError("an earlier PR delivery requires reconciliation")
        current["pr_delivery"] = {
...
def _open_pr_locked(it: dict, project: dict, wt: Path) -> str:
    ...
    if delivery.get("state") == "complete":
        ...
        if not receipt.get("url"):
            raise BossError("completed PR delivery has no exact receipt")
        return receipt["url"]
    if delivery.get("state") not in _OPEN_PR_DELIVERY:
        if project.get("authority", 0) < 2 or project.get("mode") == "local-only":
            raise BossError("PR delivery refused: current registered authority does not permit opening a PR")
    ...
    if (delivery.get("project_id") != project.get("id")
        ...
        raise BossError("PR delivery journal no longer matches its project/worktree")
    ...
    if remote_head not in (None, delivery["head_sha"]):
        raise BossError("remote item branch changed; refusing to overwrite it")
    if state == "armed":
    ...
    if state == "push-requested" and remote_head is None:
        ...
        if result.returncode:
            raise BossError("exact-SHA non-force push failed; PR delivery remains journaled")
        remote_head = _remote_head(project, delivery["branch"], delivery["origin"])
    if state == "push-requested":
        if remote_head != delivery["head_sha"]:
            raise BossError("remote did not prove the journaled exact SHA after push")
    ...
    if state != "pr-create-requested":
        raise BossError("PR delivery reached an unknown journal phase")
    if not create_now:
        ...
        raise BossError("PR create was already requested but no exact PR is currently observable; refusing automatic replay")
    _validate_pr_side_effect(item, project, delivery, wt)
    ...
    if not receipt:
        ...
        raise BossError(reason + "; delivery remains journaled")
    completed = _complete_pr_delivery(item, receipt)
    return completed["pr_delivery"]["pr_receipt"]["url"]
...
def open_pr(it: dict, project: dict, wt: Path) -> str:
    ...
    with locked(authority_lock()):
        ...
        if (project.get("id") != current.get("id")
            ...
            raise BossError("PR delivery refused: caller project binding is stale or mismatched")
...
def _arm(item: dict, project: dict, kind: str, verification: dict,
    ...
    def mutate(current):
        if current.get("status") != item.get("status") or current.get("head_sha") != item.get("head_sha"):
            raise BossError("promotion inputs changed before the transaction could arm")
        prior = current.get("promotion") or {}
        if prior.get("state") in _OPEN_PROMOTION:
            raise BossError("an earlier promotion transaction requires reconciliation")
        current["promotion"] = {
...
def _mark_requested(item: dict) -> dict:
    ...
    def mutate(current):
        ...
        if promotion.get("id") != intent_id or promotion.get("state") != "armed":
            raise BossError("promotion transaction changed before its side effect")
...
def _record_attempt(work_id: str, intent_id: str, observation: dict | None,
    ...
    def mutate(current):
        ...
        if promotion.get("id") != intent_id or promotion.get("state") not in _OPEN_PROMOTION:
            raise BossError("promotion transaction changed during reconciliation")
        attempt = {"at": now()}
...
def _mark_merged(item: dict, note: str, evidence: dict) -> dict:
    ...
    def mutate(current):
        ...
        if promotion.get("id") != intent_id or promotion.get("head_sha") != head:
            raise BossError("promotion transaction identity changed before merge recording")
...
def _cleanup(item: dict, project: dict) -> dict:
    ...
    def phase(name: str, **details) -> dict:
        ...
        def mutate(current):
            ...
            if active.get("id") != intent_id or current.get("status") != "merged":
                raise BossError("promotion cleanup identity changed")
    ...
    if cleanup_phase not in {"agent-quiesced", "worktree-removal-requested", "worktree-removed"}:
        ...
        if current.get("session") and not herdr.close_agent_tab(current["session"]):
            ...
            raise BossError("merge is proven, but cleanup was refused because the implementer is not positively quiesced")
    ...
    if foreign_locations:
        ...
        raise BossError("merge is proven, but cleanup found the branch in another worktree; preserving it")
    if wt.exists():
        ...
        if (actual.get("sha") != promotion.get("head_sha") or actual.get("dirty") != []
            ...
            raise BossError("merge is proven, but cleanup was refused because unlanded work appeared after approval; worktree preserved")
        ...
        raise BossError("merge is proven, but the worktree disappeared before cleanup could journal removal")
    ...
    if cleanup_phase != "worktree-removed":
        ...
        if worktree.branch_worktrees(project, item["branch"]):
            ...
            raise BossError("clean worktree removal could not be proven; branch and state were preserved")
        ...
        if current_branch != promotion.get("head_sha"):
            ...
            raise BossError("item branch changed during cleanup; the ref was preserved")
...
def _reconcile(item: dict, project: dict) -> dict | None:
    ...
    if (promotion.get("project_id") != project.get("id")
        ...
        raise BossError("promotion journal no longer matches the registered item/project")
    ...
    raise BossError("promotion journal has an unknown kind")


def promote(it: dict, project: dict, confirm: bool) -> dict:
    """Boss's explicit word, serialized and journaled; never called by the daemon."""
    from . import forge, registry, supervisor, work

    if not confirm:
        raise BossError("promotion needs --confirm (the boss's explicit word)")
    with locked(authority_lock()):
        if supervisor.away_status().get("enabled"):
            raise BossError("promotion refused while away mode is enabled; return first")
        item = work.load(it["id"])
        project = registry.get(item["project"])
        if project["authority"] < 3:
            raise BossError(f"project '{project['id']}' authority {project['authority']} < 3: raise it with "
                            f"`bossctl set {project['id']} --authority 3` if you really want bossctl merging")
        ...
        if reconciled:
            ...
            if promotion.get("base_ref") != project.get("base"):
                raise BossError("promotion remains unresolved and the configured base ref changed")
            if promotion.get("gate_provider") != project.get("gate", "native"):
                raise BossError("promotion remains unresolved and the fail-closed gate provider changed")
            ...
            if armed_policy is not None and armed_policy != current_policy:
                raise BossError("promotion remains unresolved and the registered project policy changed")
            gates.require_execution(project.get("gate", "native"), project["path"])
            if promotion.get("state") == "armed":
                ...
                raise BossError("armed promotion is not merged and cannot be replayed safely")
            ...
            if promotion.get("kind") == "github":
                if observation.get("classification") != "checks-green" or not observation.get("evidence_complete"):
                    raise BossError(f"promotion remains unresolved: {observation.get('classification')} — {observation.get('reason', '')}")
                ...
                raise BossError("local promotion outcome is unresolved; exact base evidence changed")
        else:
            ...
            if kind == "github":
                ...
                if (evidence.get("expected") or {}).get("head_sha") != item.get("head_sha"):
                    raise BossError("GitHub observation no longer matches the item head SHA")
                verification, _ = _validate(item, project)
            else:
                repo = Path(project["path"])
                if git(repo, "status", "--porcelain", "--untracked-files=all"):
                    raise BossError(f"{repo} has uncommitted changes; commit or stash them before arming promotion")
                current = git(repo, "symbolic-ref", "--short", "HEAD", check=False)
                if current != project["base"]:
                    raise BossError(f"{repo} is on '{current}', not base '{project['base']}'")
        ...
        if (project.get("authority", 0) < 3
            ...
            raise BossError("promotion authority or project/gate policy changed after arming")
        ...
        if promotion["kind"] == "github":
            ...
            if (not promotion.get("github_merge_policy")
                ...
                raise BossError("GitHub merge policy/base/head evidence changed after promotion armed")
            ...
            if result.returncode:
                raise BossError(f"GitHub merge outcome is not proven ({observation.get('classification')}): "
        ...
        repo = Path(project["path"])
        if git(repo, "status", "--porcelain", "--untracked-files=all"):
            raise BossError(f"{repo} has uncommitted changes; exact local promotion remains armed")
        current = git(repo, "symbolic-ref", "--short", "HEAD", check=False)
        if current != project["base"]:
            raise BossError(f"{repo} is on '{current}', not base '{project['base']}'")
        ...
        if base_after != promotion["head_sha"]:
            raise BossError(f"fast-forward outcome is not the reviewed exact SHA: {(result.stderr or result.stdout).strip()[-500:]}")
