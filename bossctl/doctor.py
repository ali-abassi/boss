...
def _run(args: list[str], *, cwd: Path | str | None = None, timeout: int = 10,
    ...
    except (OSError, subprocess.TimeoutExpired, BossError) as exc:
        detail = exc.msg if isinstance(exc, BossError) else f"{type(exc).__name__}: {exc}"
...
def _herdr_agent(name: str, identity: dict | None = None) -> dict:
    ...
    session = os.environ.get("HERDR_SESSION") or "boss"
...
def _owned_json_nofollow(path: Path, *, max_bytes: int = 1_000_000) -> dict:
    """Read one boss-selected JSON file without a symlink/swap window."""
    ...
    try:
        ...
        try:
            ...
            if stat.st_uid != os.getuid() or not 0 < stat.st_size <= max_bytes:
                raise BossError("authentication source ownership or size is unsafe")
        ...
        raise BossError(f"authentication source is unreadable: {type(exc).__name__}")
    if not isinstance(value, dict):
        raise BossError("authentication source must be a JSON object")
    return value
...
def _isolated_pi_probe(*, include_auth: bool = False):
    ...
    with tempfile.TemporaryDirectory(prefix="boss-doctor-pi-") as raw:
...
def _item_schema(value: object, directory_name: str) -> str | None:
    ...
    if (value.get("status") not in statuses or not isinstance(value.get("project"), str)
            ...
            or not isinstance(value.get("branch", f"boss/{directory_name}"), str)
        ...
        if state == "dead":
            if work_id in item_by_id:
                ...
                branch = _git(Path(project.get("path", "/nonexistent")), "show-ref", "--verify", "--quiet",
                              f"refs/heads/boss/{work_id}").returncode == 0 if project else False
        ...
        if wt.exists() and not incomplete_recovery:
            ...
            if branch.returncode or branch.stdout.strip() != expected:
                ...
                if project and recovery.get("state") != "corrupt":
                    ...
                    except (OSError, BossError) as exc:
    ...
    for project_id, project in projects["projects"].items():
        ...
        if local.returncode:
            ...
            add(f"project:{project_id}:checkout", "warning" if dirty.stdout.strip() else "ok",
                "boss checkout is dirty (preserved)" if dirty.stdout.strip() else "boss checkout is clean")
...
def repair(*, confirm: bool, network: bool = True, auth_source: str | None = None,
    ...
    if not confirm:
        raise BossError("repair plan is read-only until explicitly confirmed with `pi-boss doctor --repair --confirm`")
    applied, skipped = [], []
    with locked(home() / "doctor-repair.lock"):
            ...
            if name == "tighten-state-home-permissions":
                ...
                def close_launch(item):
                    ...
                    if (not launch or launch.get("role") != "reviewer"
                        ...
                        raise BossError("reviewer launch evidence changed during repair")
                ...
                except BossError: skipped.append({**action, "skip": "evidence changed"})
                ...
                def pause(item):
                    ...
                    if (item.get("status") != "running" or lease.get("pid") != expected_pid or
                        ...
                        raise BossError("dead-item evidence changed during repair")
                    if item.get("session"):
                ...
                with locked(home() / "scope-claims.lock"):
                    ...
                    if name == "hold-stale-claim":
                            ...
                            try:
                                def bind_legacy_token(item):
                                    ...
                                    if (item.get("status") == "running"
                                        ...
                                        raise BossError("legacy recovery item evidence changed")
                        ...
                        branch = (_git(Path(project["path"]), "show-ref", "--verify", "--quiet",
                                       f"refs/heads/boss/{target}").returncode == 0) if project.get("path") else False
        ...
        # boss-supplied target in addition to --repair --confirm.
        if auth_source:
            provider = str(auth_provider or "")
            if not re.fullmatch(r"[A-Za-z0-9._-]{1,80}", provider):
                raise BossError("--auth-provider is invalid")
            source = Path(auth_source).expanduser()
            if source.is_symlink():
                raise BossError("authentication source symlinks are refused")
            ...
            if not isinstance(source_data.get(provider), dict):
                raise BossError("authentication source has no safe matching provider record")
            ...
            if pi_dir.exists() and (pi_dir.is_symlink() or not pi_dir.is_dir() or pi_dir.resolve().parent != home()):
                raise BossError("destination Pi configuration directory is unsafe")
            ...
            with locked(pi_dir / "auth.lock"):
                ...
                if error or not isinstance(current, dict) or destination.is_symlink():
                    raise BossError("destination authentication record is malformed; refusing overwrite")
        ...
        for spec in model_specs or []:
            ...
            if not separator or phase not in dispatch.PHASES or not re.fullmatch(r"[^/\s]+/[^\s]+", model):
                raise BossError(f"invalid --model {spec!r}; use PHASE=provider/model")
            available, inventory_error = _offline_model_inventory()
            if inventory_error:
                raise BossError(f"model inventory is unavailable ({inventory_error}); reconciliation failed closed")
            if model not in available:
                raise BossError(f"replacement model {model!r} is not in the authoritative inventory")
        ...
        for spec in test_specs or []:
            ...
            if not separator or not command.strip():
                raise BossError(f"invalid --test {spec!r}; use PROJECT=COMMAND")
            ids.project(project_id)
            syntax = _run(["bash", "-n", "-c", command])
            if syntax.returncode:
                raise BossError(f"test command for {project_id} is not shell-valid")
        ...
        for project_id in fetch_projects or []:
            with locked(authority_lock()):
                ...
                if _run(["git", "check-ref-format", "--branch", base]).returncode:
                    raise BossError(f"project {project_id} has an unsafe base ref")
                ...
                if not origin:
                    raise BossError(f"project {project_id} has no integrity-checked origin URL")
                ...
                if result.returncode:
                    raise BossError(f"fetch for {project_id} failed; network uncertainty was not treated as freshness")
                if after_head != before_head or after_status != before_status:
                    raise BossError(f"fetch for {project_id} changed the checkout unexpectedly; inspect it manually")
                remote = _git(repo, "rev-parse", f"refs/remotes/origin/{base}")
                if remote.returncode:
                    raise BossError(f"fetch for {project_id} produced no exact remote-base receipt")
...
def render(report: dict) -> str:
    ...
    if report.get("repairs"):
        lines += ["", f"repair plan: {len(report['repairs'])} explicitly confirmable non-destructive action(s)",
                  "  inspect JSON: pi-boss doctor --json",
                  "  apply:        pi-boss doctor --repair --confirm"]
    return "\n".join(lines)
