...
from .util import BossError, locked, now, git_config, project_sh
...
def monitor_item(work_id: str, *, force: bool = True) -> dict:
    ...
    if not force and previous.get("observed_at"):
        ...
        try: interval = max(1, int(os.environ.get("BOSS_FORGE_INTERVAL_SECONDS", DEFAULT_INTERVAL_SECONDS)))
...
def require_green(work_id: str) -> dict:
    ...
    if (observation.get("classification") != "checks-green" or not observation.get("evidence_complete")
        ...
        raise BossError(f"GitHub merge refused: {observation.get('classification')} — {observation.get('reason')}")
    return observation
