...
PASS_ENV = ("BOSS_HOME", "BOSS_PIW", "PI_CODING_AGENT_DIR")
...
def cli_required(*args: str) -> dict:
    ...
    if out is None:
        ...
        raise BossError(f"Herdr operation failed: {' '.join(args[:3])}")
    return out
...
def _result(out: dict, key: str) -> dict:
    ...
    if not isinstance(value, dict):
        from .util import BossError
        raise BossError(f"Herdr response did not contain a real {key} identity")
    return value
...
def _verify_runtime_attestation(agent: dict, model: str, thinking: str, *,
    ...
    try:
        if path.is_symlink() or not path.is_file() or path.stat().st_size > 64_000:
            raise BossError("Pi runtime attestation is missing, linked, or oversized")
        raw = path.read_bytes()
        data = json.loads(raw)
    except BossError:
        raise
    except (OSError, UnicodeError, json.JSONDecodeError, TypeError) as exc:
        raise BossError(f"Pi runtime attestation is unreadable: {type(exc).__name__}")
    expected = {
        "schema": 1,
    ...
    for key, value in expected.items():
        if value is None or data.get(key) != value:
            raise BossError(f"Pi runtime attestation disagrees on {key}")
    from . import sandbox
    ...
    if (sandbox_evidence.get("required") is not True
        ...
        raise BossError("Pi runtime did not prove the pinned macOS write sandbox")
    try:
        ...
        raise BossError("Pi runtime attestation has no verifiable Ed25519 event key") from exc
    try:
        pid = int(data.get("pid"))
        session_dir = Path(str(data.get("session_dir"))).resolve()
        session_file = Path(str(data.get("session_file"))).resolve()
    except (TypeError, ValueError, OSError):
        raise BossError("Pi runtime attestation contains invalid process/session paths")
    if pid <= 0 or session_file.parent != session_dir or not session_file.name.endswith(f"_{data['session_id']}.jsonl"):
        raise BossError("Pi runtime attestation contains an escaped or invalid session file")
    ...
    if (not isinstance(process_identity, dict) or process_identity.get("pid") != pid
        ...
        raise BossError("Pi runtime attestation has no exact process-birth identity")
    ...
    if process_evidence.get("state") != "live":
        raise BossError("Pi runtime process identity is not live: " +
    ...
    if events_path != expected_events or events_path.parent != path.parent or events_path.is_symlink() or not events_path.is_file():
        raise BossError("Pi runtime event ledger is missing or escaped its attestation directory")
    ...
    if (path_stat.st_uid != os.getuid() or events_stat.st_uid != os.getuid()
        ...
        raise BossError("Pi runtime evidence ownership, permissions, or link count is unsafe")
    herdr_evidence = data.get("herdr") or {}
    if herdr_evidence.get("pane_id") and herdr_evidence.get("pane_id") != agent.get("pane_id"):
        raise BossError("Pi runtime attestation is bound to a different Herdr pane")
    ...
    if pid not in foreground_pids:
        raise BossError("Pi runtime attestation PID is not the live Herdr pane process")
    digest = hashlib.sha256(raw).hexdigest()
    if agent.get("attestation_sha256") and agent["attestation_sha256"] != digest:
        raise BossError("Pi runtime attestation changed after acceptance")
...
def exact_agent_liveness(identity: dict, evidence: dict | None = None, *,
    ...
    try:
        ...
        if not runtime:
            raise BossError("durable Pi runtime attestation is missing")
        ...
        if (not isinstance(pinned_process, dict)
            ...
            raise BossError("durable Pi process-birth identity is missing or changed")
        ...
        if (session.get("agent_session_id") is not None
            ...
            raise BossError("durable Pi session file reports a different UUID")
    except (BossError, OSError, ValueError, TypeError) as exc:
...
def _runtime_events(agent: dict) -> list[dict]:
    ...
    try:
        ...
        if (path.is_symlink() or not path.is_file() or stat.st_size > 16_000_000
            ...
            raise BossError("Pi runtime event ledger is missing, linked, or oversized")
        raw = path.read_text()
    except BossError:
        raise
    except (OSError, UnicodeError) as exc:
        raise BossError(f"Pi runtime event ledger is unreadable: {type(exc).__name__}")
    try:
        ...
        raise BossError("Pi runtime event ledger has no attested Ed25519 key") from exc
    ...
    for line in raw.splitlines():
        try:
            ...
            raise BossError("Pi runtime event ledger contains invalid JSON")
        ...
        try:
            ...
            raise BossError("Pi runtime event ledger signature is invalid") from exc
        if (event.get("event_sequence") != event_sequence or event.get("previous_sha256") != previous_sha
                or event.get("event_sha256") != digest):
            raise BossError("Pi runtime event ledger hash chain is invalid")
        previous_sha = digest
        if (event.get("schema") != 1 or event.get("nonce") != agent.get("attestation_nonce") or
            ...
            raise BossError("Pi runtime event ledger contains invalid or foreign evidence")
        if event_type == "input":
            if (sequence != latest_input + 1 or lifecycle.get(sequence) is not None or
                ...
                raise BossError("Pi runtime event ledger contains an invalid input sequence")
            latest_input = sequence; lifecycle[sequence] = "input"
        elif sequence > latest_input or lifecycle.get(sequence) is None:
            raise BossError("Pi runtime event ledger contains lifecycle evidence without an input")
        elif event_type == "agent_start":
            if lifecycle[sequence] != "input":
                raise BossError("Pi runtime event ledger contains a duplicate or reordered start")
            lifecycle[sequence] = "started"
        else:
            if lifecycle[sequence] != "started":
                raise BossError("Pi runtime event ledger contains a duplicate or reordered settle")
    ...
    if anchor_sequence:
        if (len(events) < anchor_sequence
            ...
            raise BossError("Pi runtime event ledger rolled back or forked behind its controller-owned anchor")
    return events
...
def _durable_runtime_anchor(agent: dict) -> tuple[int, str | None]:
    ...
    for record in candidates:
        ...
        if (not isinstance(sequence, int) or isinstance(sequence, bool) or sequence <= 0
            ...
            from .util import BossError
            raise BossError("controller-owned runtime anchor is malformed")
        if sequence in by_sequence and by_sequence[sequence] != digest:
            from .util import BossError
            raise BossError("controller-owned runtime anchors conflict")
...
def record_runtime_anchor(agent: dict) -> dict:
    ...
    def pin(item):
        ...
        if not targets:
            from .util import BossError
            raise BossError("runtime anchor has no matching durable agent identity")
        for target in targets:
            ...
            if prior_sequence > sequence or (prior_sequence == sequence and prior_sha not in (None, digest)):
                from .util import BossError
                raise BossError("runtime anchor attempted to move backward or fork")
...
def validate_agent(agent: dict, model: str, thinking: str, *, require_durable: bool = False) -> None:
    ...
    if actual_model is None or actual_thinking is None:
        raise BossError("Herdr agent omitted model/thinking metadata; refusing an unverifiable session")
    if actual_model != model or str(actual_thinking) != thinking:
        raise BossError(f"agent drift: resolved {model} thinking={thinking}, got {actual_model} thinking={actual_thinking}")
    if not (agent.get("agent_session_id") or evidence.get("agent_session_id")):
        raise BossError("Herdr agent omitted its durable session identity")
    if agent.get("agent_session_id") and evidence.get("agent_session_id") and agent["agent_session_id"] != evidence["agent_session_id"]:
        raise BossError("Herdr session identity disagrees with the durable Pi session record")
    if not runtime and not evidence.get("agent_session_id"):
        raise BossError("agent identity has neither Pi runtime attestation nor durable session evidence")
    if require_durable:
        if not (evidence.get("agent_session_id") and _reported_model(evidence) and evidence.get("thinking")):
            raise BossError("Pi did not materialize durable session evidence after the agent turn")
...
def _reserve_recovery(item: dict, session_id: str) -> None:
    ...
    if (authorization.get("consumed_at") or authorization.get("started_at") or
        ...
        from .util import BossError
        raise BossError("implementer is dead; explicit `bossctl recover` authorization is required before replacement")
    if _durable_item(item):
        def reserve(current):
            ...
            if (current_auth.get("consumed_at") or current_auth.get("started_at") or
                ...
                from .util import BossError
                raise BossError("recovery authorization was already consumed or changed")
...
def _reserve_launch(item: dict, record: dict) -> None:
    ...
    def reserve(current):
        ...
        if conflict:
            from .util import BossError
            raise BossError(f"unreconciled {record['role']} launch {conflict.get('launch_id')} blocks a duplicate")
...
def _update_launch(item: dict, launch_id: str, **changes) -> None:
    ...
    def mutate(current):
        ...
        if not launch:
            from .util import BossError
            raise BossError("agent launch journal entry disappeared")
...
def _finalize_launch(item: dict, launch_id: str, session: dict, *, reviewer: bool) -> None:
    ...
    def finalize(current):
        ...
        if not launch or launch.get("state") not in {"reserved", "tab-created", "attested"}:
            from .util import BossError
            raise BossError("agent launch journal changed before identity finalization")
        ...
        if prior and prior.get("agent_session_id") != session.get("agent_session_id"):
            if replacement_for != prior.get("agent_session_id"):
                from .util import BossError
                raise BossError("session changed while replacement was launching")
        ...
        if replacement_for:
            if (authorization.get("session_id") != replacement_for or authorization.get("consumed_at") or
                ...
                from .util import BossError
                raise BossError("recovery authorization changed before replacement identity was durable")
...
def _reconnect_reserved_launch(item: dict, launch: dict, model: str, thinking: str) -> dict:
    ...
    if launch.get("model") != model or launch.get("thinking") != thinking:
        raise BossError("reserved agent launch disagrees with the frozen model/thinking decision")
    ...
    if liveness.get("state") != "live":
        raise BossError("reserved Herdr launch liveness is unknown; refusing a duplicate agent")
    agent = {**launch, **liveness["agent"], "agent_cwd": launch.get("cwd"),
             "attestation_nonce": launch.get("attestation_nonce")}
    runtime = _verify_runtime_attestation(agent, model, thinking)
    if not runtime:
        raise BossError("reserved live Pi agent emitted no runtime attestation")
...
def ensure_agent(item: dict, cwd: Path, model: str, thinking: str, *, reviewer: bool = False,
    ...
    if not inside():
        from .util import BossError
        raise BossError("persistent execution requires Herdr (start with pi-boss); no fallback session was fabricated")
    if _durable_item(item):
    ...
    if not reviewer and existing.get("agent_name"):
        ...
        if live:
            ...
            if existing.get("agent_session_id") and live.get("agent_session_id") != existing["agent_session_id"]:
                from .util import BossError
                raise BossError("Herdr agent name now refers to a different session; refusing identity drift")
        if live:
            return {**existing, **live, "node": node or existing.get("node"),
                    "reconnected": True, "liveness_validated_at": now()}
        from .util import BossError
        if liveness.get("state") != "dead":
            raise BossError("existing Herdr session liveness is unknown; refusing an automatic replacement")
        ...
        if pending:
            if reviewer:
                from .util import BossError
                raise BossError(f"unreconciled reviewer launch {pending.get('launch_id')} blocks a fresh reviewer")
    ...
    from . import sandbox
    exact_writes = [value for value in str((agent_env or {}).get("BOSS_AGENT_ALLOWED_WRITES") or "").splitlines()
                    if value]
    ...
    pane_env = {key: value for key, value in (agent_env or {}).items()
                if (key == "GIT_CONFIG_COUNT" or key.startswith("GIT_CONFIG_KEY_")
                    or key.startswith("GIT_CONFIG_VALUE_") or key == "BOSS_AGENT_ALLOWED_WRITES")}
    pane_env["GIT_OPTIONAL_LOCKS"] = "0"
    tab = open_tab(label, "", cwd,
                   restore_focus=False, extra_env={**pane_env,
                       "BOSS_AGENT_ATTESTATION": str(attestation_path),
                       "BOSS_AGENT_EVENTS": str(events_path),
                       "BOSS_AGENT_NONCE": nonce,
                       "BOSS_AGENT_ROOT": str(cwd.resolve()),
                       "BOSS_SANDBOX_REQUIRED": "1",
                       "BOSS_SANDBOX_PROFILE": sandbox_record["profile"],
                       "BOSS_SANDBOX_PROFILE_SHA256": sandbox_record["profile_sha256"],
                       "BOSS_TOOL_SANDBOX_PROFILE": sandbox_record["tool_profile"],
                       "BOSS_TOOL_SANDBOX_PROFILE_SHA256": sandbox_record["tool_profile_sha256"],
                       "BOSS_TOOL_RUNNER": sandbox_record["tool_runner"],
                       "BOSS_SANDBOX_PROBE": sandbox_record["probe"],
                       "BOSS_REAL_PI": sandbox_record["real_pi"],
    ...
    if not tab:
        from .util import BossError
        raise BossError("Herdr could not create an agent tab")
    ...
    if not shell_roundtrip(tab["pane_id"]):
        close_tab(tab["tab_id"]); focus_tab(return_tab)
        from .util import BossError
        raise BossError("Herdr agent tab did not complete an interactive shell round trip")
    if not bind_pi_wrapper(tab["pane_id"], str(Path(sandbox_record["wrapper_dir"]) / "pi")):
        close_tab(tab["tab_id"]); focus_tab(return_tab)
        from .util import BossError
        raise BossError("Herdr agent tab could not bind the managed Pi capability wrapper")
    args = ["agent", "start", name, "--kind", "pi", "--pane", tab["pane_id"], "--timeout", "60000",
            ...
            "--extension", str(REPO / "bossctl" / "pi_attest.ts")]
    try:
        ...
        if not runtime:
            from .util import BossError
            raise BossError("Pi emitted no runtime attestation")
...
def prompt_agent(target: str, text: str, timeout: int, identity: dict | None = None) -> dict:
    ...
    if identity and sequence:
        ...
        if finding:
            from .util import BossError
            raise BossError("unexpected control finding while waiting for agent")
        return agent
    return wait_agent(target, timeout)


def prompt_agent_async(target: str, text: str, identity: dict | None = None) -> dict:
    """Prove input submission, then restore the boss without waiting for completion."""
    from .util import BossError
    text = str(text).strip()
    if not text:
        raise BossError("refusing to submit an empty agent prompt")
    before = agent_liveness(target)
    if before.get("state") != "live":
        raise BossError("cannot submit input without a positively live Herdr agent")
    if identity:
        exact = exact_agent_liveness(identity, before)
        if exact.get("state") != "live" or exact.get("identity_verified") is not True:
            raise BossError("cannot submit input without the exact durable Pi identity")
...
def _wait_runtime_monitored_inner(target: str, timeout: int, monitor, identity: dict,
    ...
    while time.monotonic() < deadline:
        ...
        # deliver before queued boss guidance has actually settled.
        ...
        if _runtime_settled(identity, sequence):
            ...
            if not live:
                raise BossError("Pi settled but Herdr liveness became unknown")
...
def steer_agent(target: str, text: str, timeout: int = 300,
    ...
    return prompt_agent_async(target, "STEERING FROM THE BOSS:\n" + text, identity)
...
def interrupt_agent(target: str, identity: dict | None = None) -> None:
    if identity:
        ...
        if exact.get("state") != "live" or exact.get("identity_verified") is not True:
            from .util import BossError
            raise BossError("refusing to interrupt an unproven or replaced Pi identity")
    workspace = os.environ.get("HERDR_WORKSPACE_ID")
    return_tab = focused_tab(workspace) if workspace else os.environ.get("HERDR_TAB_ID")
    cli_required("agent", "focus", target)
    try:
...
def open_tab(label: str, command: str, cwd: Path | None = None, *, restore_focus: bool = True,
    ...
    """Create a tab beside the boss and run `command` in it. Returns {tab_id, pane_id}."""
    workspace = os.environ["HERDR_WORKSPACE_ID"]
    return_tab = focused_tab(workspace)
    pane_environment: dict[str, str] = {}
    for k in PASS_ENV:
        v = os.environ.get(k) or (str(home()) if k == "BOSS_HOME" else None)
        if v:
            pane_environment[k] = v
    for key, value in sorted((extra_env or {}).items()):
        if not re.fullmatch(r"[A-Z][A-Z0-9_]*", key) or "\x00" in value:
            from .util import BossError
            raise BossError("invalid Herdr pane environment request")
    ...
    if restore_focus:
        ...
        # it continues independently when the boss regains focus.
        time.sleep(0.05)
        focus_tab(return_tab)
