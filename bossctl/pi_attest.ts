...
// record after its first assistant event; BOSS validates it separately.
...
export function scopeViolation(toolName: string, input: any, rootValue?: string,
  ...
  if (!rootValue) return "BOSS worktree boundary is missing";
  ...
  if (/(?:^|[^\\])~(?:\/|\b)|\$(?:\{)?(?:HOME|BOSS_HOME|PI_CODING_AGENT_DIR|CODEX_HOME|BOSS_AGENT_)/.test(command)) {
...
export default function attest(pi: ExtensionAPI) {
  const destination = process.env.BOSS_AGENT_ATTESTATION;
  const eventsPath = process.env.BOSS_AGENT_EVENTS;
  const nonce = process.env.BOSS_AGENT_NONCE;
  const agentRoot = process.env.BOSS_AGENT_ROOT;
  const allowedWrites = (process.env.BOSS_AGENT_ALLOWED_WRITES ?? "").split("\n").filter(Boolean);
  const sandboxRequired = process.env.BOSS_SANDBOX_REQUIRED === "1";
  const sandboxProfileSha256 = process.env.BOSS_SANDBOX_PROFILE_SHA256;
  const toolSandboxProfile = process.env.BOSS_TOOL_SANDBOX_PROFILE;
  const toolSandboxProfileSha256 = process.env.BOSS_TOOL_SANDBOX_PROFILE_SHA256;
  const toolRunner = process.env.BOSS_TOOL_RUNNER;
  const toolSandboxTmp = process.env.TMPDIR;
  const sandboxProbe = process.env.BOSS_SANDBOX_PROBE;
  let inputSequence = 0;
  ...
  pi.on("session_start", async (event: any, ctx: any) => {
    ...
    if (!destination || !eventsPath || !nonce || !ctx.model) {
      throw new Error("incomplete BOSS runtime-attestation request");
    }
    ...
    for (const key of ["BOSS_AGENT_ATTESTATION", "BOSS_AGENT_EVENTS", "BOSS_AGENT_NONCE",
                       "BOSS_AGENT_ROOT", "BOSS_AGENT_ALLOWED_WRITES", "BOSS_SANDBOX_REQUIRED",
                       "BOSS_SANDBOX_PROFILE_SHA256", "BOSS_TOOL_SANDBOX_PROFILE",
                       "BOSS_TOOL_SANDBOX_PROFILE_SHA256", "BOSS_TOOL_RUNNER",
                       "BOSS_SANDBOX_PROBE", "BOSS_HOME"]) {
  ...
  pi.on("tool_call", async (event: any) => {
    ...
    if (reason) return { block: true, reason: `BOSS blocked tool call: ${reason}` };
...
// Deterministic guard checks without starting Pi. `bun bossctl/pi_attest.ts`
if (process.argv[1]?.endsWith("pi_attest.ts")) {
  const root = "/tmp/boss-owned";
  const allowed = ["/tmp/boss-state/verdict.json"];
  ...
  ok(Boolean(scopeViolation("bash", { command: "echo x > $BOSS_AGENT_EVENTS" }, root, allowed)), "ledger variable blocked");
