...
``high-assurance`` is First Mate's native workflow. ``no-mistakes`` is a
separate optional product and is reachable only through the pinned external
transaction adapter in :mod:`helm.no_mistakes`.
"""
...
def require_execution(provider: str, repo: Path | str) -> dict:
    ...
    if not evidence.get("ready"):
        raise HelmError(f"gate provider {provider} is unavailable: {evidence.get('reason', 'unverified')}")
    return evidence
...
def require_receipt(provider: str, item: dict, project: dict) -> dict:
    ...
    raise HelmError(f"unknown gate provider {provider}")
