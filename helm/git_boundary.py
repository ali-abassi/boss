...
def _fail(message: str):
    # Import lazily: util delegates here, and util owns HelmError.
...
def _config_file_entries(path: Path) -> list[tuple[str, str]]:
    ...
    scratch = Path(tempfile.mkdtemp(prefix="firstmate-config-")); scratch.chmod(0o700)
    try:
...
def run_git(args: list[str], *, cwd: Path | str | None = None, check: bool = True,
    ...
    scratch = Path(tempfile.mkdtemp(prefix="firstmate-controller-git-"))
    scratch.chmod(0o700)
    try:
...
def run_project(args: list[str], *, cwd: Path | str, check: bool = True,
    ...
    scratch = Path(tempfile.mkdtemp(prefix="firstmate-controller-project-")); scratch.chmod(0o700)
    try:
        safe_env = _minimal_env(scratch=scratch, network=network, inherited=env,
...
def local_config_values(repo: Path | str, key: str) -> list[str]:
    ...
    scratch = Path(tempfile.mkdtemp(prefix="firstmate-git-config-")); scratch.chmod(0o700)
    try:
