...
def _read() -> dict:
    try: value = json.loads(_path().read_text())
        ...
        raise HelmError(f"memory ledger is unreadable: {type(exc).__name__}; run `pi-firstmate doctor`")
    if (not isinstance(value, dict) or value.get("version") != VERSION
            or not isinstance(value.get("entries"), list) or len(value["entries"]) > MAX_ENTRIES):
        raise HelmError("memory ledger schema is unsupported or malformed")
    seen = set()
    for entry in value["entries"]:
        ...
        if (not isinstance(entry, dict) or entry.get("scope") != "operational"
                ...
                or entry.get("source") != "captain-explicit-cli"):
            raise HelmError("memory ledger contains a malformed, duplicated, or untrusted entry")
        seen.add(key)
    return value
...
def _key(value: str) -> str:
    ...
    if not KEY_RE.fullmatch(key):
        raise HelmError("memory key must be 1-80 lowercase letters, digits, dots, dashes, or underscores")
    if key in FORBIDDEN_KEYS or any(word in key for word in ("transcript", "conversation", "session-jsonl")):
        raise HelmError("transcripts and conversation/session dumps are not memory entries")
    return key


def _value(value: str) -> str:
    text = value.strip()
    if not text: raise HelmError("memory value cannot be empty")
    if len(text) > MAX_VALUE:
        raise HelmError(f"memory value exceeds {MAX_VALUE} characters; store a narrow fact, not a transcript")
    if control.redact(text) != text:
        raise HelmError("memory value appears to contain a credential or secret")
    if _looks_like_transcript(text):
        raise HelmError("memory value looks like a chat/session transcript; store one narrow fact instead")
    return text
...
def get_operational(key: str) -> dict:
    ...
    if not entry: raise HelmError(f"unknown operational memory key '{key}'")
    return dict(entry)
    ...
    with locked(_lock()):
        ...
        if entry:
            ...
            if len(data["entries"]) >= MAX_ENTRIES:
                raise HelmError(f"operational memory is at its {MAX_ENTRIES}-entry bound; remove an entry first")
            entry = {"id": f"operational:{key}", "scope": "operational", "key": key, "value": value,
                     "created": timestamp, "updated": timestamp, "revision": 1,
                     "source": "captain-explicit-cli"}
...
def remove_operational(key: str, *, confirm: bool) -> dict:
    key = _key(key)
    if not confirm:
        raise HelmError("removing operational memory requires --confirm")
    with locked(_lock()):
        data = _read(); before = len(data["entries"])
        data["entries"] = [entry for entry in data["entries"] if entry.get("key") != key]
        if len(data["entries"]) == before: raise HelmError(f"unknown operational memory key '{key}'")
        write_json(_path(), data)
...
def request_project(project_id: str, operation: str, key: str, value: str | None = None, *, confirm: bool = False) -> dict:
    from . import work
    if operation not in ("set", "remove"): raise HelmError("project memory operation must be set or remove")
    key = _key(key)
    if operation == "set": value = _value(value or "")
    elif not confirm: raise HelmError("removing project memory requires --confirm")
    desired = {"scope": "project", "project": project_id, "operation": operation, "key": key,
               "value": value, "canonical_file": "AGENTS.md", "source": "captain-explicit-cli"}
    ...
    if active:
        ...
        raise HelmError(f"project memory key '{key}' already has unresolved item {latest['item_id']}; resolve it before updating")
