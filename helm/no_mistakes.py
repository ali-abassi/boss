...
First Mate journals intent before invoking it, reads authoritative receipts from
...
from .util import HelmError, git, git_config, now
...
def _repo_binding(repo: Path | str, nm_home: Path) -> dict:
    ...
    # First Mate's GitHub lifecycle currently binds head and base to one exact
...
def status(repo: Path | str | None = None, *, binary: Path | str | None = None,
    ...
    except (AdapterError, OSError, sqlite3.Error, HelmError) as exc:
...
def _receipt_classification(item: dict, project: dict, tx: dict, run: dict,
    ...
    if final_head != tx.get("submitted_head_sha"):
        return {"classification": "changed-head",
                "reason": "no-mistakes produced a different head; explicit custody sync and fresh First Mate verification are required",
...
def inspect(item: dict, project: dict) -> dict:
    ...
    except (HelmError, AdapterError, OSError, sqlite3.Error, KeyError) as exc:
        return {"provider": "no-mistakes", "classification": "unknown",
...
def _validate_item(item: dict, project: dict, wt: Path, capability: dict) -> dict:
    ...
    if project.get("gate") != "no-mistakes":
        raise HelmError("item is not configured for the no-mistakes provider")
    if item.get("project") != project.get("id") or item.get("status") != "running":
        raise HelmError("item/project state does not permit a no-mistakes submission")
    ...
    if binding.get("default_branch") != project.get("base"):
        raise HelmError("no-mistakes default branch does not match the registered First Mate base")
    ...
    if not (_full_sha(head_sha) and _full_sha(base_sha) and verification.get("ok") is True
        ...
        raise HelmError("no-mistakes submission requires complete exact-SHA First Mate verification")
    if git(wt, "rev-parse", "HEAD", check=False) != head_sha or not worktree.is_clean(wt):
        raise HelmError("no-mistakes submission worktree is not clean at the reviewed exact SHA")
    if git(project["path"], "rev-parse", project["base"], check=False) != base_sha:
        raise HelmError("registered base moved before no-mistakes submission")
    if worktree.signature(wt) != verification.get("fingerprint"):
        raise HelmError("worktree fingerprint changed before no-mistakes submission")
    return verification
...
def arm(item: dict, project: dict, wt: Path) -> dict:
    ...
    if not capability.get("ready"):
        raise HelmError(f"gate provider no-mistakes is unavailable: {capability.get('reason', 'unverified')}")
    ...
    if not intent:
        raise HelmError("no-mistakes submission requires the original task intent")
    ...
    def mutate(current):
        if current.get("status") != "running" or current.get("head_sha") != item.get("head_sha"):
            raise HelmError("item changed before the external gate could arm")
        previous = current.get("external_gate") or {}
        if previous.get("state") in OPEN_TRANSACTION_STATES:
            raise HelmError("an earlier no-mistakes transaction requires reconciliation")
        if previous:
...
def _validate_before_command(item: dict, project: dict, *, allow_needs_you: bool = False) -> tuple[dict, Path]:
    ...
    if _project_policy(current_project) != tx.get("project_policy") or _project_policy(project) != _project_policy(current_project):
        raise HelmError("project policy changed after the external gate armed")
    allowed_statuses = {"running", "needs-you"} if allow_needs_you else {"running"}
    if item.get("status") not in allowed_statuses:
        raise HelmError("item state no longer permits the external gate command")
    ...
    if unresolved or controls.get("pause_requested") or controls.get("interrupt_requested"):
        raise HelmError("unresolved captain control prevents an external gate command")
    ...
    if wt != expected_wt or not wt.is_dir():
        raise HelmError("journaled external-gate worktree is missing or changed")
    if git(wt, "rev-parse", "HEAD", check=False) != tx.get("submitted_head_sha") or not worktree.is_clean(wt):
        raise HelmError("external-gate worktree changed after arming")
    if worktree.signature(wt) != tx.get("worktree_fingerprint"):
        raise HelmError("external-gate worktree fingerprint changed after arming")
    if git(project["path"], "rev-parse", project["base"], check=False) != tx.get("base_sha"):
        raise HelmError("registered base moved after the external gate armed")
    try:
        _attestation_still_matches(tx)
    except AdapterError as exc:
        raise HelmError(str(exc)) from exc
...
def _update(work_id: str, transaction_id: str, expected_states: set[str], **changes) -> dict:
    ...
    def mutate(current):
        ...
        if tx.get("id") != transaction_id or tx.get("state") not in expected_states:
            raise HelmError("external gate journal changed during reconciliation")
        tx.update(changes)
...
def _invoke_journaled(item: dict, project: dict, args: list[str], *, response_id: str | None = None) -> dict:
    ...
    def mark_requested(value):
        ...
        if active.get("id") != transaction_id or active.get("state") not in expected_state:
            raise HelmError("external gate journal changed before the CLI request could start")
        ...
        if response_id:
            ...
            if not response or response.get("state") != "armed":
                raise HelmError("external gate response journal changed before the CLI request could start")
    ...
    def started(pid: int) -> None:
        ...
        def record(value):
            ...
            if active.get("id") != transaction_id or active.get("state") != requested_state:
                raise HelmError("external gate journal changed after CLI start")
    ...
    def record_result(value):
        active = value.get("external_gate") or {}
        if active.get("id") != transaction_id or active.get("state") != requested_state:
            raise HelmError("external gate journal changed before CLI result recording")
...
def reconcile(work_id: str) -> dict:
    ...
    if tx.get("provider") != "no-mistakes":
        raise HelmError("item has no no-mistakes transaction to reconcile")
    ...
    def record(value):
        ...
        if active.get("id") != transaction_id:
            raise HelmError("external gate identity changed during receipt reconciliation")
        if result.get("run_id"):
            if active.get("run_id") not in {None, result["run_id"]}:
                raise HelmError("external gate run identity changed during reconciliation")
...
def start_or_reconcile(item: dict, project: dict, wt: Path) -> dict:
    ...
    if tx.get("provider") == "no-mistakes" and not matches_current and tx.get("state") in OPEN_TRANSACTION_STATES:
        raise HelmError("an unresolved no-mistakes transaction is bound to an older item revision")
...
def respond(work_id: str, action: str, findings: list[str] | None = None,
    ...
    if action not in {"approve", "fix", "skip"}:
        raise HelmError("external gate action must be approve, fix, or skip")
    findings = [value.strip() for value in (findings or []) if value.strip()]
    item = work.load(work_id); project = registry.get(item["project"])
    tx = item.get("external_gate") or {}
    if item.get("status") != "needs-you" or tx.get("state") != "needs-decision":
        raise HelmError("item is not waiting at a reconciled no-mistakes decision gate")
    ...
    if observation.get("classification") != "needs-you" or not gate.get("fingerprint"):
        raise HelmError("the journaled external gate is no longer authoritatively observable")
    ...
    if action == "fix" and (not findings or not set(findings) <= available):
        raise HelmError("fix requires a non-empty subset of the exact observed finding IDs")
    if action != "fix" and (findings or instructions):
        raise HelmError("finding IDs and instructions are valid only with action=fix")
    ...
    def arm_response(current):
        ...
        if active.get("id") != tx.get("id") or active.get("state") != "needs-decision":
            raise HelmError("external gate changed before the response could arm")
        response = {
...
def require_receipt(item: dict, project: dict) -> dict:
    ...
    if result.get("classification") != "checks-passed":
        raise HelmError(f"no-mistakes exact-SHA receipt is not complete: {result.get('reason', 'unknown')}")
    tx = item.get("external_gate") or {}
    if (tx.get("state") != "complete" or item.get("head_sha") != result.get("head_sha")
            or item.get("pr_url") != result.get("pr_url")):
        raise HelmError("no-mistakes receipt is not durably bound to the current item")
    return result
