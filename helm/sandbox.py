...
def _restricted_profile(*, writable: list[Path], exact_writes: list[Path],
    ...
    # Candidate tools do not need any other user's-home or HELM state. System
...
def available() -> dict:
    ...
    tool_runner = REPO / "libexec" / "firstmate-tool"
...
def prepare(*, work_id: str, launch_id: str, cwd: Path, session_dir: Path,
    ...
    for controlled in (session_dir, attestation_dir):
        try: controlled.relative_to(item_root)
            ...
            raise HelmError("sandbox-owned session evidence escaped the item state directory")
    ...
    # must never receive write authority over First Mate's shared Pi home, so
    ...
    if not source.is_dir() or source.is_symlink():
        raise HelmError("managed Herdr agents require a real isolated Pi configuration directory")
    copied: list[str] = []
    for name in ("auth.json", "settings.json", "models.json", "models-store.json"):
        ...
        if not candidate.is_file() or candidate.is_symlink() or candidate.stat().st_uid != os.getuid():
            raise HelmError(f"refusing unsafe Pi configuration source: {name}")
        destination = pi_dir / name
        shutil.copyfile(candidate, destination)
        destination.chmod(0o600)
        copied.append(name)
    if not copied:
        raise HelmError("managed Herdr agents found no usable Pi configuration files")
    ...
    for raw in exact_writes or []:
        ...
        if target == cwd or cwd in target.parents:
            ...
            try: target.relative_to(item_root)
                ...
                raise HelmError("sandbox exact-write capability escaped item state")
            literals.append(target)
...
def run_verification(*, work_id: str, phase: str, cwd: Path, command: str,
    ...
    status = available()
    if not status["ready"]:
        raise HelmError("candidate verification requires the pinned macOS sandbox; refusing an unsandboxed test")
