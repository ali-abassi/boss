...
from .util import HelmError, locked, now, write_json
...
def _strict_json(path: Path, default: dict) -> dict:
    try:
        ...
        raise HelmError(f"supervisor state is unreadable at {path}: {type(exc).__name__}; run `pi-firstmate doctor`")
    if not isinstance(value, dict):
        raise HelmError(f"supervisor state at {path} is not an object; run `pi-firstmate doctor`")
    return value
...
def _state() -> dict:
    ...
    if error := validate_state(value):
        raise HelmError(f"malformed supervisor state ({error}); run `pi-firstmate doctor`")
...
def _queue() -> dict:
    ...
    if error := validate_queue(value):
        raise HelmError(f"malformed wake queue ({error}); run `pi-firstmate doctor`")
    return value
...
def classify(item: dict, *, at: float | None = None, agent: dict | None = None) -> dict:
    ...
    if status in ATTENTION or (status == "paused" and item.get("ask")):
        return {"classification": "needs-you", "reason": f"item state {status} requires captain action", "age_seconds": age}
    ...
    if status == "running":
        ...
        if age >= _threshold("HELM_SUPERVISOR_STALE_SECONDS", DEFAULT_STALE_SECONDS):
    ...
    if status in ("queued", "paused"):
        ...
        if wait_until and at >= wait_until:
            return {"classification": "needs-you", "reason": f"declared wait elapsed at {_iso(wait_until)}", "age_seconds": age,
                    "declared_wait_elapsed": True}
        if status == "queued" and age is not None and age >= _threshold("HELM_SUPERVISOR_QUEUE_STALE_SECONDS", DEFAULT_QUEUE_STALE_SECONDS):
...
def _enqueue(queue: dict, item: dict, observation: dict, key: str, epoch: float) -> bool:
    ...
    if len(queue["events"]) >= MAX_RECORDS:
        raise HelmError("wake queue reached its durable bound; no pending decision was discarded")
...
def _observe_locked(items: list[dict], state: dict, queue: dict, epoch: float, probe_agents: bool) -> list[dict]:
    results = []
    wedge_after = _threshold("HELM_SUPERVISOR_WEDGE_OBSERVATIONS", DEFAULT_WEDGE_OBSERVATIONS)
    resurface = _threshold("HELM_SUPERVISOR_WEDGE_RESURFACE_SECONDS", DEFAULT_WEDGE_RESURFACE_SECONDS)
    for item in items:
        work_id = item.get("id")
        if not work_id:
            continue
        ...
        # accumulate forever in a long-running fleet. Remove their old record;
        ...
        if item.get("status") in TERMINAL:
            ...
            if work_id not in state["observations"] and len(state["observations"]) >= MAX_RECORDS:
                raise HelmError("supervisor observation ledger reached its bound without dropping live state")
...
def claim(consumer: str, *, limit: int = 20, at: float | None = None) -> list[dict]:
    if not consumer.strip():
        raise HelmError("wake consumer identity is required")
    epoch = time.time() if at is None else at
    with locked(supervisor_lock()):
        state, queue = _state(), _queue()
        ...
        for event in queue["events"]:
            ...
            # A crash leaves a durable uncertain receipt for the captain/doctor
...
def _delivery_update(ids: list[str], consumer: str, state: str, *, at: float | None = None) -> int:
    if state not in ("sending", "sent"):
        raise HelmError("invalid wake delivery transition")
...
def set_away(enabled: bool) -> dict:
    ...
    with locked(authority_lock()):
        ...
        if blocking:
            ...
            raise HelmError(f"away mode refused: resolve supervisor attention first ({names})")
        from . import work
        ...
        if unresolved_transactions:
            raise HelmError("away mode refused: reconcile open transaction(s) first (" +
        ...
        if not report.get("healthy"):
            ...
            raise HelmError("away mode refused: doctor recovery gate failed (" + ", ".join(errors[:8]) + ")")
        live_workers = [c for c in report.get("checks", []) if c.get("id", "").startswith("worker:") and c.get("status") == "ok"]
        if not live_workers:
            raise HelmError("away mode refused: no positively live worker is registered")
        ...
        with locked(supervisor_lock()):
            ...
            if pending_count:
                raise HelmError(f"away mode refused: {pending_count} pending decision wake(s) must be handled first")
...
def parse_duration(value: str) -> int:
    ...
    if not match or not any(match.groups()):
        raise HelmError("duration must look like 30s, 20m, 1h, or 1h30m")
    seconds = int(match.group(1) or 0) * 3600 + int(match.group(2) or 0) * 60 + int(match.group(3) or 0)
    if seconds <= 0:
        raise HelmError("duration must be positive")
    return seconds
    ...
    if duration in (None, "clear"):
        ...
        def set_wait(item):
            item["declared_wait"] = {"declared_at": _iso(epoch), "until": _iso(epoch + seconds),
                                     "reason": reason.strip() or "captain-declared wait"}
