...
from helm import supervisor
from helm.util import HelmError, write_json


class AwayTests(unittest.TestCase):
    def setUp(self):
        self.home = Path(tempfile.mkdtemp()); os.environ["HELM_HOME"] = str(self.home)
    ...
    def enable(self):
        with mock.patch("helm.supervisor.scan", return_value=[]), mock.patch("helm.doctor.audit", return_value=self.good_report):
            return supervisor.set_away(True)

    def test_enable_requires_clean_supervision_doctor_and_live_worker(self):
        with mock.patch("helm.supervisor.scan", return_value=[{"item_id": "x", "classification": "unknown"}]), \
             mock.patch("helm.doctor.audit", return_value=self.good_report):
            with self.assertRaises(HelmError): supervisor.set_away(True)
        bad = {**self.good_report, "healthy": False, "checks": [{"id": "state:wakes", "status": "error"}]}
        with mock.patch("helm.supervisor.scan", return_value=[]), mock.patch("helm.doctor.audit", return_value=bad):
            with self.assertRaises(HelmError): supervisor.set_away(True)
        no_worker = {**self.good_report, "checks": []}
        with mock.patch("helm.supervisor.scan", return_value=[]), mock.patch("helm.doctor.audit", return_value=no_worker):
            with self.assertRaises(HelmError): supervisor.set_away(True)
    ...
    def test_open_external_transaction_blocks_away_even_for_terminal_item(self):
        ...
        write_json(directory / "item.json", {
            ...
            "created": "2026-01-01T00:00:00Z", "updated": "2026-01-01T00:00:00Z",
            "branch": "firstmate/p-merged", "worktree": str(self.home / "worktrees" / "p" / "p-merged"),
