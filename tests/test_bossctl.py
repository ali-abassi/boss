...
class BossctlTests(unittest.TestCase):
    def setUp(self):
    ...
    def task(self, text="add a file", **kw):
        ...
        for k, v in kw.items():
            args += [f"--{k}", str(v)]
        return json.loads(self.bossctl(*args).stdout)

    def show(self, wid):
        return json.loads(self.bossctl("show", wid, "--json").stdout)
    ...
    def test_local_only_ready_then_promote_ff(self):
        ...
        r = self.bossctl("promote", it["id"], check=False)
        self.assertEqual(r.returncode, 1)
        self.bossctl("promote", it["id"], "--confirm")
        log = subprocess.run(["git", "-C", str(self.proj), "log", "--oneline"], capture_output=True, text=True).stdout
        self.assertIn("bossctl: fake change", log)
    ...
    def test_authority_gates_promotion(self):
        ...
        it = self.task(); self.bossctl("run-once")
        r = self.bossctl("promote", it["id"], "--confirm", check=False)
        self.assertEqual(r.returncode, 1); self.assertIn("authority 1 < 3", r.stderr)
        r = self.bossctl("task", "p", "x", "--kind", "ship", check=False)  # fine at authority 1
        self.assertEqual(r.returncode, 0)
        self.bossctl("set", "p", "--authority", "0")
        r = self.bossctl("task", "p", "x", check=False)
    ...
    def test_ask_goes_to_inbox_and_respond_requeues_with_guidance(self):
        ...
        self.assertIn("guided", (wt / "bossctl-change.txt").read_text())

    def test_failures_requeue_with_notes_then_exhaust(self):
        self.add(mode="local-only")
        it = self.task(**{"max-attempts": 2})
        self.bossctl("run-once", mode="fail", check=False)
        it = self.show(it["id"])
        ...
        self.bossctl("retry", it["id"]); self.bossctl("run-once", mode="ok")
    ...
    def test_dirty_ship_and_mutating_scout_fail_without_losing_work(self):
        ...
        self.bossctl("run-once", check=False)
    ...
    def test_cancel_discard_never_touches_a_running_item(self):
        ...
        refused = self.bossctl("cancel", item["id"], "--discard", check=False)
        self.assertNotEqual(refused.returncode, 0)
        self.assertTrue((wt / "unreviewed.txt").is_file())
    ...
    def test_token_budget_exhaustion_pauses_without_delivery(self):
        ...
        refused = self.bossctl("resume", item["id"], check=False)
        self.assertNotEqual(refused.returncode, 0); self.assertIn("exhausted budget", refused.stderr)
        self.bossctl("budget", item["id"], "--tokens", "300")
        self.bossctl("resume", item["id"])
    ...
    def test_active_headless_pause_stops_runner_and_preserves_recoverable_worktree(self):
        ...
        proc = subprocess.Popen(BOSS + ["run-once"], env=env, text=True,
                                stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        deadline = time.time() + 10
        while time.time() < deadline and self.show(item["id"])["status"] != "running":
            time.sleep(0.05)
        self.bossctl("pause", item["id"])
        ...
        self.assertTrue((self.home / "worktrees" / "p" / item["id"]).exists())
        self.bossctl("resume", item["id"]); self.bossctl("run-once")
    ...
    def test_headless_review_evidence_must_name_the_exact_sha(self):
        ...
        for value in ("omit", "wrong"):
            ...
            r = subprocess.run(BOSS + ["run-once"], env={**self.env, "FAKE_PIW_REVIEW_SHA": value},
    ...
    def test_later_commit_invalidates_exact_sha_reviews(self):
        ...
        self.bossctl("run-once"); item = self.show(item["id"])
    ...
    def test_moved_base_after_review_refuses_promotion(self):
        self.add(mode="local-only", authority=3)
        item = self.task("review against current base"); self.bossctl("run-once")
        ...
        result = self.bossctl("promote", item["id"], "--confirm", check=False)
    ...
    def test_base_move_during_final_test_invalidates_the_entire_pipeline(self):
        ...
        self.bossctl("set", "p", "--test", test_cmd)
    ...
    def test_confirmed_doctor_quarantine_then_recover_resumes_preserved_item(self):
        ...
        self.bossctl("doctor", "--repair", "--confirm", "--offline", check=False)
        quarantined = self.show(item["id"]); self.assertEqual(quarantined["status"], "paused")
        self.bossctl("recover", item["id"], "--request-id", "restart-recovery")
        self.bossctl("run-once")
        self.assertEqual(self.show(item["id"])["status"], "ready")
    ...
    def test_scout_writes_report_and_cleans_up(self):
        ...
        self.bossctl("run-once", mode="scout")
        it = self.show(it["id"])
    ...
    def test_per_project_concurrency_and_daemon_drain(self):
        ...
        self.bossctl("daemon", "--interval", "0", "--once-idle", "1")
    ...
    def test_stale_lease_is_preserved_and_surfaced_without_automatic_relaunch(self):
        ...
        self.bossctl("run-once")                      # observes only; does not reclaim or relaunch
        it = self.show(it["id"])
        self.assertEqual(it["status"], "running")
        wakes = json.loads(self.bossctl("wakes", "--json").stdout)
    ...
    def test_add_autodetects_test_command(self):
        ...
        r = self.bossctl("add", str(self.proj), "--id", "p", "--json")
    ...
    def test_busy_worker_is_still_supervised_by_zero_token_watcher(self):
        ...
        env = {**self.env, "FAKE_PIW_SECONDS": "4", "BOSS_SUPERVISOR_STALE_SECONDS": "1",
               "BOSS_SUPERVISOR_WEDGE_OBSERVATIONS": "2"}
        run = subprocess.run(BOSS + ["daemon", "--interval", "1", "--once-idle", "1"],
    ...
    def test_legacy_mode_writes_canonical_state_and_unavailable_external_gate_blocks_creation(self):
        ...
        project = json.loads(self.bossctl("projects", "--json").stdout)["p"]
        self.assertEqual(project["mode"], "high-assurance")
        self.bossctl("set", "p", "--gate", "no-mistakes")
        result = self.bossctl("task", "p", "must not start", check=False)
        self.assertNotEqual(result.returncode, 0)
    ...
    def test_queued_native_item_rechecks_external_gate_before_any_execution_effect(self):
        ...
        self.bossctl("set", "p", "--gate", "no-mistakes")
        result = self.bossctl("run-once", check=False)
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("gate provider no-mistakes is unavailable", result.stderr)
    ...
    def test_up_status_down(self):
        self.add(mode="local-only")
        it = self.task()
        self.bossctl("up", "--interval", "1")
        self.assertIn("team      ready", self.bossctl("status").stdout)
        deadline = time.time() + 30
        while time.time() < deadline and self.show(it["id"])["status"] != "ready":
            time.sleep(0.5)
        self.assertEqual(self.show(it["id"])["status"], "ready")
        self.assertIn("stopped", self.bossctl("down").stdout)
        self.assertIn("stopped", self.bossctl("status").stdout)
