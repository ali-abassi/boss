...
the work completes. Nobody touches the boss's checkouts until the boss says so.
"""
try:
...
class OneThreadTest(unittest.TestCase):
    def setUp(self):
        ...
        for name in ("api", "web", "docs"):
            ...
            self.bossctl("add", str(repo), "--id", name, "--test", "true", "--mode", "local-only", "--authority", "3")
    ...
    def items(self):
        return json.loads(self.bossctl("work", "--all", "--json").stdout)

    def run_two_daemons_until_drained(self):
        procs = [subprocess.Popen(BOSS + ["daemon", "--owner", f"worker-{i}", "--interval", "0", "--once-idle", "3"],
    ...
    def test_one_liaison_many_workers_one_channel(self):
        ...
        queued = [
            self.bossctl("task", "api", "add rate limiting to /login", "--json"),
            self.bossctl("task", "api", "fix flaky session test [fake:ask]", "--json"),     # this one will need a human
            self.bossctl("task", "web", "dark mode toggle", "--json"),
            self.bossctl("task", "web", "why is the bundle 4MB?", "--kind", "scout", "--json"),
            self.bossctl("task", "docs", "document the new auth flow", "--json"),
            self.bossctl("task", "docs", "audit broken links", "--kind", "scout", "--json"),
        ]
        ...
        # 4. One channel back. Everything the boss needs is in the single inbox — nowhere else.
        inbox = self.bossctl("inbox", "--hints").stdout   # the BOSS reads it with ids
        ...
        # 5. Workers never wrote to the boss's checkouts. Only the boss's word moves main.
        ...
        # 6. The liaison relays the boss's answer; the same worker pool finishes the job.
        self.bossctl("respond", asker, "use the existing OAuth provider")
        self.run_two_daemons_until_drained()
        it = json.loads(self.bossctl("show", asker, "--json").stdout)
        ...
        self.assertEqual(self.bossctl("promote", asker, check=False).returncode, 1)
        self.bossctl("promote", asker, "--confirm")
        self.assertNotEqual(git(self.projects["api"], "rev-parse", "HEAD"), heads_before["api"])
        self.assertEqual(json.loads(self.bossctl("show", asker, "--json").stdout)["status"], "merged")
